...
async function startPaper() {
  ...
  try {
    ...
    message.value = '已提交 paper 启动请求；等待 worker/模拟引擎状态确认'
...
async function stopPaper() {
  ...
  try {
    await api.stopPaper()
    message.value = '已提交 paper 停止请求；等待 worker 状态确认'
    await load()
  } catch (error) {
...
async function resetPaper() {
  ...
  try {
    ...
    message.value = '已提交 paper 重置请求；等待状态确认，历史订单数据库不被伪造清空'
...
async function submitOrder() {
  ...
  try {
    ...
    message.value = '已提交 paper 订单，等待 worker/撮合状态；不代表已成交'
...
async function cancelOrder(order: Row) {
  ...
  try {
    ...
    message.value = '已提交 paper 撤单请求，等待 worker 状态确认'
    pendingCancel.value = ''
    await load()
  } catch (error) {
...
async function closePosition(position: Row) {
  ...
  try {
    ...
    message.value = `已提交 ${position.code} 的 paper 平仓请求，等待 worker/撮合状态`
    pendingClose.value = ''
    await load()
  } catch (error) {
...
async function saveRiskRules() {
  ...
  try {
    ...
    message.value = '已提交 paper 风控规则更新，等待 worker 状态确认'
    await load()
  } catch (error) {
