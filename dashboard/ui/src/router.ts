...
const routes = [
  ...
  { path: '/app/reports/:id', component: () => import('./views/ReportDetailView.vue'), meta: { title: '报告详情' } },
  ...
  { path: '/app/risk', redirect: (to: RouteLocationGeneric) => ({ path: '/app/portfolio-risk', query: mergedLegacyQuery(to, new URLSearchParams(), 'legacy-risk') }), meta: { title: '风险监控' } },
  ...
  { path: '/app/ai/runtime', redirect: (to: RouteLocationGeneric) => ({ path: '/app/ai', query: { ...mergedLegacyQuery(to, new URLSearchParams(), 'legacy-ai-runtime'), tab: 'settings' } }), meta: { title: 'AI Runtime 配置' } },
