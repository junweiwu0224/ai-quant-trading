...
- OpenClaw gateway 在 Docker Compose 中仍使用 `--auth none`，并发布宿主机端口 `18789`。
...
2. `scripts/deployment_static_preflight.py` 必须把缺失的生产风险决策文档视为 hard finding；OpenClaw `--auth none` 和 `18789` 端口发布在未解决前继续作为 soft finding。
3. OpenClaw auth/network、真实 provider、外部 LLM/OpenClaw、数据同步/迁移、paper/live/broker/trading 不允许通过本地预检自动放行。
4. 若上线时选择临时接受 OpenClaw `--auth none`、公开端口、真实 provider 限制或交易边界风险，必须记录 owner、到期时间、补偿控制、回滚方式和后续修复项；不得无限期默认接受。
...
- 接受的代价：上线前多一道人工签收和证据记录；在 soft finding 未解决时，生产推进会被 `--fail-on-soft` 或人工发布审查拦住。
...
- 负面影响 / 风险：
  ...
  - 如果团队选择接受 soft finding，仍必须用网络隔离、反向代理认证、防火墙或短期内修复来降低风险。
- 需要同步修改的地方：
  - `docs/production-readiness-runbook.md`
  - `docs/release-evidence/2026-06-12-local-delivery-readiness.md`
...
- 若 OpenClaw auth/network 被永久修复，更新本 ADR 或新增 superseding ADR，并把相关 soft finding 从静态预检中降噪或移除。
