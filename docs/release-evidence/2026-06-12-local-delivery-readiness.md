...
- Stock diagnosis prompt-contract bridge, exposing a redacted local `stock_diagnosis_prompt_contract_v1` allowlist/forbidden-claim contract while keeping `llm_status=not_invoked`.
- Stock diagnosis local explanation draft, exposing a deterministic local `stock_diagnosis_local_explanation_v1` summary/notes/gap snapshot backed by the same cited rows and prompt contract while keeping `llm_status=not_invoked`.
...
- Local release bundle generated and verified: `13` files plus `manifest.json` in `releases/local-delivery-2026-06-12/local-delivery-2026-06-12.tar.gz`.
...
- Latest default local release preflight: context pack OK, release evidence OK, pytest `860 passed, 1 warning`, compileall passed, `git diff --check` passed.
...
- Event-study statistical validation boundary checks passed locally: JS syntax checks passed for `alpha-tools.js`, `app.js`, `core/app-shell.js`, `app-ui-shell.js`, and `sw.js`; focused basket backend tests reported `14 passed, 1 warning`; focused draft-audit/cache-busting/frontend contracts reported `6 passed, 1 warning`; targeted `compileall` for `alpha/basket.py` and `dashboard/routers/alpha.py` passed.
- Event-study release gates passed locally: release evidence check passed, `git diff --check` passed, local bundle rebuilt with `13` files plus `manifest.json`, bundle verify-only passed, and default release preflight passed with `855 passed, 1 warning`, compileall, and diff-check.
- Event-study p-value/BH correction checks passed locally: JS syntax checks passed for `alpha-tools.js`, `app.js`, `core/app-shell.js`, `app-ui-shell.js`, and `sw.js`; focused basket backend tests reported `16 passed, 1 warning`; focused draft-audit/cache-busting/frontend contracts reported `6 passed, 1 warning`; targeted `compileall` for `alpha/basket.py` and `dashboard/routers/alpha.py` passed; default release preflight passed with pytest `857 passed, 1 warning`, compileall, and diff-check.
- Event-study local holdout checks passed locally: JS syntax checks passed for `alpha-tools.js`, `app.js`, `core/app-shell.js`, `app-ui-shell.js`, and `sw.js`; focused basket backend tests reported `18 passed, 1 warning`; focused draft-audit/cache-busting/frontend contracts reported `6 passed, 1 warning`; targeted `compileall` for `alpha/basket.py` and `dashboard/routers/alpha.py` passed; default release preflight passed with pytest `859 passed, 1 warning`, compileall, and diff-check; local release bundle rebuild and verify-only passed. The backend now distinguishes same-date cross-sectional samples from time-ordered holdout samples and only marks local `out_of_sample_validated=true` when a later entry-date holdout keeps the selected metric direction positive.
- Event-study explicit event-sample bridge checks passed locally: JS syntax checks passed for `stock-detail-core.js`, `core/app-shell.js`, `app.js`, `app-ui-shell.js`, and `sw.js`; focused basket backend tests reported `19 passed, 1 warning`; focused event-group/AppShell/draft-audit/cache-busting frontend contracts reported `7 passed, 1 warning`; targeted `compileall` for `alpha/basket.py` and `dashboard/routers/alpha.py` passed; default release preflight passed with pytest `860 passed, 1 warning`, compileall, and diff-check; local release bundle rebuild and verify-only passed. The backend now accepts caller-provided `conditions.event_samples[]`, audits each valid sample by its own stock/date, reports invalid sample count, and keeps global `event_date` fallback for legacy drafts.
...
- In-app Browser event-study QA: temporary local Dashboard at `127.0.0.1:8001` rendered `#research` basket subtab at desktop `1280x900` and mobile `390x844` without new page console errors or horizontal overflow. Fresh full navigation loaded `app.js?v=139`, `app-ui-shell.js?v=50`, `core/app-shell.js?v=39`, `alpha-tools.js?v=14`, `style.css?v=86`, and `/sw.js?v=79`. Initial hash-only navigation stayed on the browser's previously cached shell, so the QA used `?qa=task936` full navigation to verify the current bundle.
...
- Market sector news/research bridge focused checks passed locally: JS syntax and targeted Python compileall passed; focused sector-members API contracts reported `4 passed, 1 warning`; focused intelligence market frontend/cache contracts reported `2 passed, 1 warning`. `/api/market/sector-members` now returns `news_research.status=local_candidate` when locally cached market news matches sector/theme/constituent context, with `provider_verified=false`, `local_only=true`, and a coverage note that board-level research coverage, provider completeness, and news freshness are not verified. Cache-busted resources are `app.js?v=148`, `app-ui-shell.js?v=59`, `intelligence-market.js?v=29`, `/sw.js?v=88`, and `ai-quant-v196`.
...
- Stock diagnosis local-explanation checks passed locally: JS syntax and targeted Python compileall passed; focused stock-detail backend explanation checks reported `3 passed, 1 warning`; focused stock AI rail/cache contracts reported `4 passed, 1 warning`; context pack passed; default release preflight passed with pytest `861 passed, 1 warning`, compileall, and diff-check; local release bundle rebuild and verify-only passed. The detail payload now carries `diagnosis_evidence.local_explanation` with schema `stock_diagnosis_local_explanation_v1`, deterministic dimension notes, evidence gaps, risk disclaimer, forbidden-claim inheritance, and `llm_status=not_invoked`; the AI rail shows the local explanation summary without invoking an LLM. Cache-busted resources are `app.js?v=146`, `app-ui-shell.js?v=57`, `stock-detail-core.js?v=30`, `/sw.js?v=86`, and `ai-quant-v194`.
- Latest default local release preflight after the prompt-contract slice passed: context pack OK, release evidence OK, pytest `861 passed, 1 warning`, compileall passed, and `git diff --check` passed.
- Local release bundle was rebuilt after the prompt-contract slice and `scripts/build_release_bundle.py --verify-only` passed against the current manifest/checksums.
- In-app Browser stock local-explanation QA: local `#stock` page rendered at `127.0.0.1:8001` on desktop default viewport and mobile `390x844`, loaded cache-busted resources `app.js?v=146`, `app-ui-shell.js?v=57`, and `stock-detail-core.js?v=30`, displayed the stock input and evidence-rail shell without page console errors or horizontal overflow. Concrete local-explanation rendering is covered by TestClient and Node DOM contracts because browser smoke did not obtain a live authenticated stock-detail payload with deterministic fixtures.
- In-app Browser stock prompt-contract QA: authenticated local `#stock` page rendered at `127.0.0.1:8001`, loaded `app.js?v=145` and `app-ui-shell.js?v=56`, displayed the stock empty state and evidence-rail shell without page console errors. Concrete prompt-contract rendering is covered by TestClient and Node DOM contracts because the browser smoke did not fetch an authenticated stock-detail payload with deterministic local fixtures.
...
- Stock diagnosis prompt-contract bridge is local contract shaping and rendering only; it redacts secret-like values, excludes provider/header/cookie/credential inputs, keeps `invocation_allowed=false`, and does not call external LLM/OpenClaw/provider services or produce an explanation/recommendation/trading-readiness claim.
- Stock diagnosis local explanation draft is deterministic local evidence shaping only; it summarizes cited rows, gaps, and risk disclaimer under the prompt contract, keeps `llm_status=not_invoked`, and does not call external LLM/OpenClaw/provider services or produce recommendation/trading-readiness claims.
