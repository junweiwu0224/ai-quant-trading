...
- Latest default local release preflight: context pack OK, release evidence OK, pytest `855 passed, 1 warning`, compileall passed, `git diff --check` passed.
...
- Event-study statistical validation boundary is audit metadata and frontend disclosure only; it does not compute or claim p-values, does not perform multiple-testing correction or out-of-sample validation, and does not execute strategy/backtest/trading paths automatically.
