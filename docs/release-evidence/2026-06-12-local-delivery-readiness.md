...
- Stock workbench event-flow, same-day event-group detail preview, basket/backtest draft, and local event-study audit evidence.
...
- Stock workbench same-day event groups now render a compact detail preview inside the bottom event center. The preview keeps the representative `主事件详情` distinct from a manually clicked `选中事件详情`, shows source/time/detail, independent/raw evidence counts, duplicate-source hints, and preserves manual-only explain/basket/backtest draft actions.
...
- In-app Browser stock QA: temporary local Dashboard at `127.0.0.1:8001` rendered `#stock` at desktop `1280x900` and mobile `390x844` without page console errors or horizontal overflow, and loaded cache-busted resources `app.js?v=135`, `stock-detail-core.js?v=22`, and `style.css?v=84`. Event-group preview state is covered by the Node DOM contract because the in-app Browser read-only page scope cannot construct internal workbench state.
...
- Stock event-group detail preview is local frontend rendering only; it does not fetch new event data, open external source links, run backtests, create baskets/watchlists automatically, submit orders, or change trading/auth/production behavior.
