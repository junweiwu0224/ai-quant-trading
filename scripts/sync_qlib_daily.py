...
def sync_qlib_daily(
    ...
    target_codes = list(codes or DEFAULT_CODES)
    ...
    for code in target_codes:
        try:
            ...
            df = _normalize_daily_frame(raw_df)
            if df.empty:
...
def main(codes: tuple[str, ...], count: int, generate_predictions: bool, min_success: int) -> None:
    """同步 Qlib 机会池所需的代表性 A 股日线。"""
    setup_logging()
    target_codes = parse_codes(codes) or DEFAULT_CODES
