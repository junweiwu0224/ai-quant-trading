...
async function submitTask(runNow = false) {
  ...
  try {
    ...
    taskMessage.value = runNow ? '任务已在当前进程运行。' : '任务已提交到独立 AI Worker 队列。'
...
<template>
  <section class="ai-workbench">
    ...
    <section class="panel ai-boundary-panel">
      ...
      <div><strong>研究权限边界</strong><p>AI 读取调用方提供的冻结快照，输出可复核的解释、筛选条件、策略草案和诊断。它不会修改确定性决策、自动推送资格、模拟盘或真实订单。</p></div>
      <div class="ai-boundary-meta"><span>运行时 {{ statusValue('runtime', '未连接') }}</span><span>决策影响 {{ statusValue('decision_effect', 'none') }}</span><span>Worker {{ workerState?.ready ? '进程已就绪' : workerState?.status || '进程未就绪' }}</span></div>
    </section>
