...
<template>
  <BrokerDisableGuard />
  <section>
    <div class="page-head"><div><RouterLink to="/app/workflows" class="muted small"><ArrowLeft :size="14" />返回工作流目录</RouterLink><h1>模拟盘与风控执行</h1><p>完整恢复旧模拟盘工作流。所有写操作明确标注为模拟盘并需要人工确认；此页不会调用 Broker 或真实下单接口。</p></div><button class="button" type="button" :disabled="loading" @click="load"><RefreshCw :size="16" :class="{ spin: loading }" />刷新</button></div>
