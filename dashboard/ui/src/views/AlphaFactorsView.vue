...
<template>
  <section>
    <div class="page-head"><div><RouterLink to="/app/research" class="muted small"><ArrowLeft :size="14" />研究工作区</RouterLink><h1>{{ mode === 'formula' ? '公式系统与篮子计划' : 'Alpha、因子与 Walk-Forward' }}</h1><p>研究操作由人工触发并保留结果；AI、模型和公式都不能直接改变确定性策略动作或自动推送资格。</p></div><button class="button" type="button" :disabled="loading" @click="load"><RefreshCw :size="16" />刷新</button></div>
