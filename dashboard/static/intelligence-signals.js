...
(function () {
    ...
    Object.assign(Intelligence, {
        ...
        renderSignalPool(data, validation = null, { pendingValidation = false } = {}) {
            ...
            const confidenceLabel = validation?.success ? confidenceText(validation.confidence) : '未验证';
            ...
            const rows = preds.map((p, i) => {
                ...
                const rowConfidence = validation?.success && validation.confidence !== 'unverified'
                ...
                return `<tr class="qlib-row ${heat.cls}" data-code="${code}">
                    ...
                    <td class="qlib-td qlib-td-actions">
                        <button class="qlib-btn qlib-btn-mimo" data-code="${code}" data-name="${name}" data-score="${score.toFixed(3)}" data-industry="${App.escapeHTML(p.industry || '--')}" title="问 AI">🤖</button>
                    </td>
                </tr>`;
            }).join('');
