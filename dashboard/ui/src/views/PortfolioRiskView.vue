...
const totalPnl = computed<number | null>(() => {
  if (!positions.value.length) return null
  const values = positions.value.map((item) => Number(item.pnl ?? item.unrealized_pnl)).filter((value) => Number.isFinite(value))
...
<template>
  <section>
    ...
    <div class="summary-strip"><div class="summary-item"><span>总权益</span><strong>{{ number(snapshot?.total_equity) }}</strong><small>当前模拟盘快照</small></div><div class="summary-item"><span>持仓市值</span><strong>{{ number(snapshot?.market_value) }}</strong><small>{{ positions.length }} 个标的</small></div><div class="summary-item"><span>组合盈亏</span><strong :class="totalPnl == null ? '' : totalPnl >= 0 ? 'good' : 'bad'">{{ number(totalPnl) }}</strong><small>由当前持仓返回值合计</small></div><div class="summary-item"><span>最大回撤</span><strong class="bad">{{ number(risk?.max_drawdown ?? snapshot?.max_drawdown) }}</strong><small>风险指标不是交易指令</small></div></div>
