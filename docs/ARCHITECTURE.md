...
| `/api/llm/*` | 5 | LLM 对话、策略生成、结果解读、研报列表、研报解读 |
| `/api/broker/*` | 4 | 券商配置 |
| `/api/system/*` | 6 | 系统状态、策略导入导出、版本对比 |
...
| # | 功能 | 对标竞品 | 说明 | 状态 |
|---|------|---------|------|------|
| 1 | 板块热力图 | 同花顺 | 行业×涨跌幅 treemap 色块可视化，红涨绿跌色阶 | ✅ 已完成 |
| 2 | 多周期共振 | 通达信 | 日/周/月三周期 MA/RSI/MACD 信号共振分析，强度评分 | ✅ 已完成 |
| 3 | 分时图增强 | 通达信 | 量比实时显示、均价线、多日分时叠加对比 | ✅ 已完成 |
| 4 | 龙虎榜深度分析 | 东方财富 Choice | 营业部画像 TOP15、上榜记录、3日上涨概率、收益统计 | ✅ 已完成 |
...
| # | 功能 | 对标竞品 | 当前状态 | 难度 |
|---|------|---------|---------|------|
| 1 | 公式系统 | 通达信 | 已交付基础 UI：类通达信语法公式引擎、选股与目录入口，后续可继续扩展自定义指标/更复杂函数 | ★★★ |
| 2 | 篮子交易 | QMT/PTrade | 已交付基础 UI：AI 选股计划生成、等权/风险平价分配、计划回测入口，后续可继续增强执行链路 | ★★ |
