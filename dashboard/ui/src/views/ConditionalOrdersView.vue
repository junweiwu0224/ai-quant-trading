...
<template>
  <BrokerDisableGuard />
  <section>
    <div class="page-head"><div><RouterLink to="/app/portfolio-risk" class="muted small"><ArrowLeft :size="14" />组合工作区</RouterLink><h1>条件单</h1><p>条件单只写入模拟盘订单路径。规则可以编辑、启停和删除；启用不等于真实下单，也不会绕过风险校验。</p></div><button class="button" type="button" :disabled="loading" @click="load"><RefreshCw :size="16" :class="{ spin: loading }" />刷新</button></div>
